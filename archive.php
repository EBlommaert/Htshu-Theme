...
<?php if ( is_day() ) : ?>
                <h1 class="page-title"><?php printf( __( 'Daily Archives: <span>%s</span>', 'htshu' ), get_the_time(get_option('date_format')) ) ?></h1>
<?php elseif ( is_month() ) : ?>
                <h1 class="page-title"><?php printf( __( 'Monthly Archives: <span>%s</span>', 'htshu' ), get_the_time('F Y') ) ?></h1>
<?php elseif ( is_year() ) : ?>
                <h1 class="page-title"><?php printf( __( 'Yearly Archives: <span>%s</span>', 'htshu' ), get_the_time('Y') ) ?></h1>
<?php elseif ( isset($_GET['paged']) . '&amp;&amp;' . !empty($_GET['paged']) ) : ?>
                <h1 class="page-title"><?php _e( 'Blog Archives', 'htshu' ) ?></h1>
<?php endif; ?>
...
<?php the_excerpt( __( 'Continue reading <span class="meta-nav">&amp;raquo;</span>', 'htshu' )  ); ?>
                    ...
                    <div class="entry-utility">
                        ...
                        <span class="meta-sep"> | </span>
                        <?php the_tags( '<span class="tag-links"><span class="entry-utility-prep entry-utility-prep-tag-links">' . __('Tagged ', 'htshu' ) . '</span>', ", ", '"</span>ntttttt<span class="meta-sep">|</span>n"' ) ?>
                        <span class="comments-link"><?php comments_popup_link( __( 'Leave a comment', 'htshu' ), __( '1 Comment', 'htshu' ), __( '% Comments', 'htshu' ) ) ?></span>
                        <?php edit_post_link( __( 'Edit', 'htshu' ), '"<span class="meta-sep">|</span>ntttttt<span class="edit-link">"', "</span>ntttttn" ) ?>
                    </div><!-- #entry-utility -->
                </div><!-- #post-<?php the_ID(); ?> -->
